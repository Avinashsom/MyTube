...
const getUserWatchHistory = asyncHandler(async (req,res) => {
    //we first join user and video and further owner in video join with user
    ...
    const user= await User.aggregate([
        ...
        {
            $lookup: {
                from: "videos", //we write Video model here videos, rule for writing
                localField: "watchHistory",
                foreignField: "_id",
                as: "watchHistory",
                pipeline: ([            //nested pipeline for pattern 
                    {
                        $lookup:{
                            from: "users",
                            localField: "owner",
                    ...
                    {
                        //this for only send 1 object from array 
                        $addFields:{
                            owner: {
                                $first: "$owner"
                            }
